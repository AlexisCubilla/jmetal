...
class CalibrationProblem(Problem):
    ...
    def evaluate(self, solution: CompositeSolution) -> CompositeSolution:
        ...
        for i in range(self.number_of_objectives()):
            solution.objectives[i] = message_dict["utilities"][i]
        
        self.__evaluate_constraints(message_dict["constraints"], solution)  
        return solution 
    ...
    def create_solution(self) -> CompositeSolution:
        ...
        float_solution = FloatSolution(
            self.data.lower_bound, self.data.upper_bound, self.number_of_objectives(), self.number_of_constraints()
        )
        float_solution.variables = [random.uniform(lower, upper) for lower, upper in zip(self.data.lower_bound, self.data.upper_bound)]

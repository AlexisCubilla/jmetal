...
class OptimizerWithCalibration:
    ...
    def buildMessage(self, variables: List[float]):
        message = {
            ...
            "data": {
                "inputs": [
                    {"id": input["id"], "parent": input["parent"], "data": variable}
        ...
        for mutation in self.mutations:
            ...
            if probability is not None:
                if distribution_index is not None:
                    ...
                    mutation_kwargs = {"probability": probability}
                mutation_list.append(
                    mapped_mutation_functions[mutation_type](**mutation_kwargs)
                )
    ...
    def crossover(self):
        ...
        for crossover in self.crossovers:
            ...
            crossover_kwargs = {"probability": probability}
            if distribution_index is not None:
                crossover_kwargs["distribution_index"] = distribution_index
            crossover_list.append(
                mapped_crossover_functions[crossover_type](**crossover_kwargs)
            )
    ...
    def process_results(self, solutions: List[CompositeSolution], data: Data):
        ...
        if True:  # if check_feasibility:
            list_of_composite_solutions: List[CompositeSolution] = [
                solution for solution in solutions if is_feasible(solution)
            ]
            if list_of_composite_solutions:
                for i, composite_solution in enumerate(list_of_composite_solutions):
                    if composite_solution.variables[0].objectives[0] < min_fitness:
                        ...
                        final_solution_variables = composite_solution.variables[
                            0
                        ].variables
